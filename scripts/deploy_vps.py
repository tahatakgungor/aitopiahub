...
def deploy():
    ...
    dirs_to_sync = ["src", "configs", "docker", "assets"]
    ...
    # Sync single files
    for f in [".env", "pyproject.toml", "poetry.lock", "alembic.ini"]:
        if os.path.exists(f):
            print(f"Uploading {f}...")
            scp.put(f, remote_path=remote_path)

    # 2. Docker build and restart
    print("🔄 Restarting Docker containers on VPS (Bilingual & Always-On mode)...")
    commands = [
